...
var setState = function (itemName, state) {
    ...
    var url = config.protocol + "://" + config.server + ":" + config.HA_port + "/rest/items/" + itemName;
    request.post(
        {
            uri: url,
            headers: {
...
var getState = function (itemName, callback) {
    var url = config.protocol + "://" + config.server + ":" + config.HA_port + "/rest/items/" + itemName + "/state";
    request({
        uri: url,
        method: "GET"
        }, function (error, response) {
